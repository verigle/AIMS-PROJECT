...
for var, title in zip(surface_vars_names, plots_titles):
    world_rmses_list, pred_dates_list, sa_rmses_list = [], [], []
    ...
    for i in range(len(selected_times) - 3):
        ...
        # Compute RMSE
        world_rmses, world_pred_dates = rmse_fn(predictions=world_predictions, target_batch=world_target_batch, var_name=var, weigths=world_rmse_weights)
        sa_rmses, sa_pred_dates = rmse_fn(predictions=sa_predictions, target_batch=sa_target_batch, var_name=var, weigths=sa_rmse_weights, area="sa")
        
        # Store results
        world_rmses_list.append(world_rmses)
        pred_dates_list.append(world_pred_dates)
        sa_rmses_list.append(sa_rmses)
        
        if (i+1) % 10 == 0:
            print(f"Iterations {i+1} for {var}")
    
    # Plot results
    plot_rmses(var, world_rmses_list, sa_rmses_list, figsize=(12, 8), fontsize=18, date_ranges=pred_dates_list, title=title, save_path="../report/hrest0", atmos_level=None)
    print("Plot Done")
