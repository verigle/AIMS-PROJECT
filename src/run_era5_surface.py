...
################### Main part#################################################################################
pred_dates_list=[]
t2_world_rmses_list=[], t2_sa_rmses_list=[]
u10_world_rmses_list=[], u10_sa_rmses_list=[]
v10_world_rmses_list=[], v10_sa_rmses_list=[]
msl_world_rmses_list=[], msl_sa_rmses_list=[]
iterations = 0
for i in range(0, len(sliced_era5_world.time)-3):
    ...
    # compute the rmse
    ## temperature
    t2_world_rmses, t2_world_pred_dates = rmse_fn(predictions=world_predictions, 
            target_batch=world_target_bacth, var_name="2t",
            weigths=world_rmse_weights)
    
    t2_sa_rmses, _ = rmse_fn(predictions=sa_predictions, 
            target_batch=sa_target_bacth, var_name="2t",
            weigths=sa_rmse_weights, area="sa")
    
    t2_world_rmses_list.append(t2_world_rmses)
    t2_sa_rmses_list.append(t2_sa_rmses)
    
    ## u10
    u10_world_rmses, u10_world_pred_dates = rmse_fn(predictions=world_predictions, 
            target_batch=world_target_bacth, var_name="10u",
            weigths=world_rmse_weights)
    
    u10_sa_rmses, _ = rmse_fn(predictions=sa_predictions, 
            target_batch=sa_target_bacth, var_name="10u",
            weigths=sa_rmse_weights, area="sa")
    
    u10_world_rmses_list.append(u10_world_rmses)
    u10_sa_rmses_list.append(u10_sa_rmses)
    
    
    pred_dates_list.append(t2_world_pred_dates)
    if iterations%10==0:
        print(f"Iterations {iterations}")

# plot for temperature    
plot_rmses("2t", t2_world_rmses_list, t2_sa_rmses_list, 
    figsize=(12, 8), fontsize=18,
    date_ranges=pred_dates_list, 
    title=plots_titles[0],
    save_path="../report/era5",
    atmos_level=None)

# u10

plot_rmses("10u", u10_world_rmses_list, u10_sa_rmses_list, 
    figsize=(12, 8), fontsize=18,
    date_ranges=pred_dates_list, 
    title=plots_titles[1],
    save_path="../report/era5",
    atmos_level=None)
