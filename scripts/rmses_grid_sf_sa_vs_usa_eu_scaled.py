...
from utils import rmse_fn, plot_rmses, create_hrest0_batch, min_max_scaled
...
for i, ax in enumerate(axs[:num_plots]):
    line1, = ax.plot(lead_time, min_max_scaled(target_region_surface_atmos_rmses[variables[i]]), label="South Africa", c="brown")
    line3, = ax.plot(lead_time, min_max_scaled(eu_region_surface_atmos_rmses[variables[i]]), label="Europe", c="navy")
    
    line2, = ax.plot(lead_time, min_max_scaled(base_region_surface_atmos_rmses[variables[i]]), label="USA", c="teal")
...
plt.savefig(f"{saving_path}/sa_vs_usa_vs_eu_scaled.pdf", bbox_inches="tight")
plt.savefig(f"{saving_path}/sa_vs_usa_vs_eu_scaled.png", bbox_inches="tight")
plt.savefig(f"{saving_path}/sa_vs_usa_vs_eu_scaled.svg", bbox_inches="tight")
